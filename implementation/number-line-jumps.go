...
func kangaroo(x1 int32, v1 int32, x2 int32, v2 int32) string {
	...
	frontKang := map[string]int32{"x": x1, "v": v1}
	backKang := map[string]int32{"x": x2, "v": v2}
	...
	if frontKang["x"] == backKang["x"] {

		if frontKang["v"] != backKang["v"] {
			return "NO"
		} else {
			return "YES"
		}
	...
	if frontKang["v"] >= backKang["v"] {
		...
		for backKang["x"] < frontKang["x"] {
			frontKang["x"] += frontKang["v"]
			backKang["x"] += backKang["v"]
		}
		if backKang["x"] > frontKang["x"] {
			return "NO"
		} else {
			return "YES"
		}
